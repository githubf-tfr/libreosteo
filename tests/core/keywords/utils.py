...
def get_date_invoice(invoice_number):
    logger.info("Retrieve the current date on invoice number = %s " % invoice_number)
    con = sqlite3.connect("data/db.sqlite3", detect_types=sqlite3.PARSE_DECLTYPES)

    cur = con.cursor()
    cur.execute(
        "SELECT date from libreosteoweb_invoice where number = ? ", (invoice_number,)
    )
    d = cur.fetchone()
...
def change_date_invoice(invoice_number, date):
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE libreosteoweb_invoice set date = ? where number = ? ",
...
def change_date_examination(examination, date):
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE libreosteoweb_examination set date = ? where id = ? ",
