...
class TestDeletePatient(APITestCase):
    def setUp(self):
        ...
        with block_disconnect_all_signal(
            ...
            self.p2 = Patient.objects.create(
                family_name="Bond", first_name="James", birth_date=datetime(1924, 1, 1)
            )
    ...
    def test_delete_patient_with_invoiced_examination_gdpr(self):
        current_invoice = Examination.objects.filter(id=self.e1.pk)[0].invoices.latest(
            "date"
        )
        assert current_invoice is not None
        response = self.client.delete(
            reverse("patient-detail", kwargs={"pk": self.p1.pk}) + "?gdpr=True"
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        ...
        assert (
            Invoice.objects.filter(id=current_invoice.id).first().patient_family_name
            == "Picard"
        )
