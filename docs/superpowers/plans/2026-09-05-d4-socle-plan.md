...
**Architecture:** onze tâches, dans l'ordre imposé des trois incréments, plus une tâche de clôture. **L'ordre interne a été révisé le 2026-09-05, en cours de lot** — PostgreSQL, puis Django, puis Python, et non plus l'inverse : le motif et le fait mesuré qui l'a provoqué sont dans la spec, § « Ce que l'exécution a établi » et § « Décisions de cadrage ». Les tâches sont **numérotées dans l'ordre d'exécution** ; leur contenu n'a pas changé. I1 (T1→T4) monte le moteur : image et point de montage, procédure de montée dans le `README.rst`, fiches de recette (purge E0 reprise, `R-INST-05` amendée, `R-INST-06` neuve), puis la montée **exécutée** depuis l'état E2. I2 (T5→T7) monte le cadre : la ligne morte du cadre, puis Django 5.2.17 et les paquets qui le suivent, puis la recette de l'incrément. I3 (T8→T10) épingle Python 3.14 : le venv et les déclarations d'abord, l'image et son serveur d'application ensuite, puis la recette de l'incrément. T11 clôt le lot.
...
- **Interpréteur de développement** : `./.venv/bin/python`. Ne jamais invoquer `python` nu, ni `pip` nu. **Jusqu'à T8 exclue, ce venv est en 3.13** — c'est l'interpréteur que `.github/workflows/main.yml` déclare, et le cliquet doit être tenu contre celui de la CI. À partir de T8, il est en **3.14**, comme la CI. Toute tâche qui trouve le venv sur un autre interpréteur que celui que la CI déclare à ce commit s'arrête et le signale. *Fait à connaître avant T1 : la tâche interrompue du 2026-09-05 a laissé `./.venv` en Python 3.14.2, où la suite unitaire est cassée (spec, § « Ce que l'exécution a établi »). T1 commence donc par le remettre en 3.13 — `uv venv --python 3.13 --clear .venv` puis les trois `uv pip install` de T8 Step 2 — sans quoi son `make check` de fin échouera pour une raison qui ne lui appartient pas.*
...
- **`target-version` et `python_version` ne sont pas des cliquets.** Ces deux valeurs du même `pyproject.toml` changent en T8 (`py313` → `py314`, `"3.13"` → `"3.14"`) : elles **suivent la cible**, elles ne l'assouplissent pas. Ne jamais les présenter comme un desserrement, ne jamais s'en autoriser un autre.
...
- **Chaque incrément laisse le produit déployable et recettable** : I1 laisse Django 4.2 sur PostgreSQL 18 et Python 3.13 (côté moteur, combinaison supportée — 4.2 ne pose qu'un plancher ≥ 12 ; côté interpréteur, c'est l'état déjà en service, que cet incrément ne touche pas), I2 laisse Django 5.2 sur PostgreSQL 18 et Python 3.13 (combinaison entièrement déclarée), I3 clôt sur Django 5.2, PostgreSQL 18 et Python 3.14. Les tâches T4, T7 et T10 sont les portes de sortie de chaque incrément : elles ne produisent aucun commit, elles produisent une preuve.
- **`$SCRATCH`** désigne, dans tout ce plan comme dans le chapitre 0 de `docs/recette.md`, un **répertoire de travail jetable hors du dépôt** — jamais une constante, jamais versionné ; en session Claude, le scratchpad de session convient. La toute première tâche, **T1**, le crée (`mkdir -p "$SCRATCH"/{db,bak,data,settings}`), et chaque incrément y dépose son relevé de preuve, nommé d'après son rang d'exécution : `preuves-i1.txt` pour I1 (T1→T4, PostgreSQL), `preuves-i2.txt` pour I2 (T5→T7, Django), `preuves-i3.txt` pour I3 (T8→T10, Python). T11 consomme les trois. Il est supprimé à la dernière étape de T11.
...
|---|---|---|---|
| `pyproject.toml` | T8 uniquement | `target-version` (l. 39) et `python_version` (l. 58). | Aucune autre tâche ne l'ouvre. **Ni `fail_under`, ni `select`, ni `ignore`, ni `files` ne sont touchés par ce lot.** |
| `Docker/build/http-ready/Dockerfile` | T9 uniquement | Les deux `FROM` (l. 6, 61), le premier `apk add` (l. 17-30), le commentaire et le second `apk add` (l. 69-88), le `CMD` (l. 128). | Aucune autre tâche ne l'ouvre. Le bloc de commentaires du `CMD` (l. 91-127) **n'est pas touché** : ses quatre explications restent vraies mot pour mot. |
| `README.rst` | T2, T8, T9 | T2 : une **section nouvelle** insérée entre le bloc `DATABASES` qui finit l. 141 et le titre « Use it in production » (l. 143). T8 : l. 37 (« Python 3.8+ ») et l. 226 (« This fork runs on Python 3.13 »). T9 : l. 202, la phrase sur `Libreosteo-sock`. | T2 → T8 → T9, l'ordre des incréments. Trois régions disjointes ; **T2 insère avant les trois autres régions, donc les lignes 202 et 226 se décalent** : repérer par le texte, jamais par le numéro. |
| `docs/recette.md` | T3 uniquement | La purge E0 (l. 189-201), l'étape 2 de `R-INST-05` (l. 570), et `R-INST-06` insérée entre la fin de `R-INST-05` (l. 631) et le titre `### Authentification` (l. 633). | Aucune autre tâche ne l'ouvre. T4, T7 et T10 **lisent** le cahier et le jouent, elles ne l'écrivent pas — sauf écart du manuel, cf. la règle « constater sans corriger ». |
| `requirements/requirements.txt` | T6 uniquement | Les lignes 1, 4, 5, 6, 9, 15, 17. | — |
| `KANBAN.md` | T11 uniquement | § « Terminé », § « À faire » / « Dette technologique », § « Points en suspens ». | — |

Aucun autre fichier n'est touché par plus d'une tâche : `.github/workflows/main.yml` n'appartient qu'à T8 ; `Docker/build/postgresql/Dockerfile`, `Docker/deploy/pg/docker-compose.yml` et `Docker/deploy/pg/.env.example` qu'à T1 ; `libreosteoweb/__init__.py` qu'à T5 ; `requirements/requ-dev.txt` qu'à T6.
...
### T1 — l'image du moteur et le point de montage
...
- Produit : `$SCRATCH` et son arborescence, sur lesquels **toutes** les tâches suivantes s'appuient (Step 4), et `$SCRATCH/preuves-i1.txt`, premier des trois relevés que T11 consomme. Produit aussi : une image `libreosteo/libreosteo-pg` en PostgreSQL 18, montée sur `/var/lib/postgresql` et non plus sur `/var/lib/postgresql/data`. Le `healthcheck` (l. 32-37) et le `depends_on: condition: service_healthy` (l. 44-46) **ne bougent pas** : la sonde TCP sur `127.0.0.1` reste juste et passe en 3 à 4 secondes sur l'image 18.
...
- [ ] **Step 4 : créer `$SCRATCH`, construire et démarrer sur un volume neuf**

T1 étant la première tâche du lot, c'est elle qui crée le répertoire de travail jetable dont toutes les suivantes se servent :
...
Puis construire les deux images sous le commit courant :
...
Consigner ces cinq sorties dans `$SCRATCH/preuves-i1.txt`. Si le point 4 met en défaut la combinaison Django 4.2 / PostgreSQL 18, **s'arrêter et journaliser le fait avant toute décision** : la fusion des incréments 1 et 2 est une révision à écrire et à motiver, pas un ajustement silencieux (spec, § « Risques »).
...
### T2 — la procédure de montée, dans le `README.rst`
...
**Interfaces:**
- Consomme : le point de montage et l'image de T1.
- Produit : **le texte que T4 exécutera sans y ajouter un geste**, et que `R-INST-06` (T3) constatera sans le paraphraser. La procédure vit ici ; la fiche en constate le résultat.

**Régime de preuve : recette et exécution réelle.** Un texte de procédure ne s'exerce d'aucun processus pytest. Sa preuve est T4 : la montée jouée depuis l'état E2, en suivant ce texte à la lettre.
...
### T3 — la recette du moteur : purge E0 reprise, `R-INST-05` amendée, `R-INST-06` neuve
...
**Interfaces:**
- Consomme : le montage de T1, la procédure de T2.
- Produit : les fiches que T4 puis T11 joueront. **Aucune fiche existante n'est renumérotée** : `R-INST-06` s'insère à la fin du domaine « Installation », les 48 fiches existantes gardent leur identifiant.

**Régime de preuve : recette.** Ces trois écritures ne se prouvent qu'en étant jouées, en T4 puis à la clôture.
...
Sur l'instance de T1, exécuter la procédure de reset telle qu'elle vient d'être écrite.
...
La remarque sur le `-c` multi-instructions est **revalidée sur le client 18 au moment de rejouer la fiche** (T4). Si le client 18 affichait les trois statuts, c'est la remarque qu'il faudrait reprendre, et c'est un écart du manuel — donc corrigible au fil de la passe, sans KO.
...
### T4 — la montée exécutée pour de vrai, et la recette de l'incrément 1
...
**Interfaces:**
- Consomme : `README.rst` (T2), `R-INST-06` (T3), les images de T1.
...
**Le lot s'exécute sur `main` : ne jamais faire un `git checkout` de `main` pour remonter dans l'historique.** L'état E2 sur PostgreSQL 13 se reconstitue dans un **arbre de travail séparé**, par `git worktree add` sur le dernier commit **antérieur à celui de T1** — c'est-à-dire, T1 étant la première tâche du lot, le commit de base sur lequel D4 a démarré : **`068c91b`**, celui où `Docker/build/postgresql/Dockerfile` porte encore `postgres:13-alpine`. Ni `git stash`, ni `git checkout`, ni `git switch` sur `main` : l'arbre principal reste sur `main`, à la pointe du lot, pendant toute la durée de cette tâche.

```sh
git show 068c91b:Docker/build/postgresql/Dockerfile | head -1   # attendu : FROM postgres:13-alpine
git worktree add "$SCRATCH/arbre-pg13" 068c91b
```

Bâtir les deux images **depuis cet arbre** et sous un tag qui lui est propre, monter l'instance, puis atteindre E1 puis E2 par le chapitre 1 de `docs/recette.md`. L'arbre de travail est retiré (`git worktree remove`) à la fin de la tâche.
...
Revenir sur l'arbre principal — celui de `main`, qui porte T1 à T3 — et jouer la fiche `R-INST-06` sans écart, en suivant le `README.rst`. L'arbre `pg13` n'est plus utilisé que comme source des images de l'ancienne majeure.
...
Consigner chaque sortie dans `$SCRATCH/preuves-i1.txt`.
...
Ajouter à `$SCRATCH/preuves-i1.txt` : les verdicts, le commit recetté, le tag d'images. Si la passe a révélé un écart **du manuel** (et non du produit), le corriger dans `docs/recette.md` et le commiter seul :
...
### T6 — Django 5.2.17 LTS et les paquets qui le suivent
...
**Interfaces:**
- Consomme : le venv 3.14 de T8, la ligne morte retirée en T5.
...
Attendu : toutes les migrations `Applying ... OK`, `WSGI app 0 (mountpoint='') ready`, `302 Found`. **Aucune migration nouvelle** n'apparaît par rapport à la passe de l'incrément 1 : `0058` reste la dernière.
...
Consigner les verdicts dans `$SCRATCH/preuves-i2.txt`, avec le commit recetté et le tag d'images. Corriger et commiter seul tout écart **du manuel**, comme en T4 Step 4.

**Critère de fin :** les dix-huit fiches en OK, `$SCRATCH/preuves-i2.txt` complet, et en particulier `R-RCH-01`/`R-RCH-02` vertes — la recherche du produit fonctionne sous `django-haystack 3.4.0`.

---

## Incrément 3 — l'interpréteur est épinglé, et il est le même partout

### T8 — le venv de développement et les déclarations passent en 3.14
...
**Interfaces:**
- Consomme : `$SCRATCH` (créé par T1) ; le cadre monté par T6 — **Django 5.2.17, qui déclare Python 3.10 à 3.14**. C'est cette dépendance qui a fait passer l'incrément Python en dernier : sous Django 4.2, cette tâche laisse la suite unitaire cassée.
- Produit : un `./.venv` en Python 3.14 sur lequel T9 et T10 s'appuient, et un `pyproject.toml` dont `ruff` et `mypy` visent 3.14. Aucune signature Python ne change.
...
Attendu : `Python 3.14.x`. **La cible est la plus haute 3.14.x que `uv` propose, et non un correctif nommé** : mesuré au 2026-09-05, `uv python install 3.14.7` et `uv python install 3.14.4` rendent tous deux `No download found`, l'index d'`uv` s'arrêtant à **3.14.2** pour cette branche quand l'image de production sert 3.14.7. L'écart de correctif entre le venv et l'image est **un fait à consigner, pas un défaut à corriger** (spec, § « Décisions de cadrage ») : ne pas le contourner par un interpréteur système, ne pas s'y arrêter. Si `uv python install` échoue faute de réseau, en revanche, s'arrêter et le signaler — à partir d'ici, aucune tâche de ce lot ne se joue sur 3.13.
...
Attendu : `make check` vert — `ruff check`, `ruff format --check`, `mypy` (`Success: no issues found in 104 source files`), `makemigrations --check` silencieux, `272 passed` et couverture ≥ 90 % ; `make test-functional` 31/31.

**Ce que cette mesure établit dans l'ordre révisé.** Elle est faite après T6, donc sur **Django 5.2.17**, et avant tout changement de déclaration : elle isole donc le seul changement d'interpréteur sur un cadre qui déclare 3.14. Elle prouve deux choses à la fois, et il faut les nommer séparément dans le relevé :

1. **que la rupture mesurée le 2026-09-05 est bien fermée** — sous Django 4.2.30, ce même Step rendait 31 échecs et deux fichiers interbloqués, `AttributeError` sur `django/template/context.py:39` (spec, § « Ce que l'exécution a établi »). `272 passed` ici est la preuve que la montée du cadre l'a refermée, et c'est la raison d'être de l'ordre révisé ;
2. **que rien d'autre ne casse sous 3.14** — toute rupture restante est un défaut préexistant que ce lot vient de rendre visible, à journaliser et à instruire, jamais à contourner.

**Attendu négatif, à vérifier explicitement** : la commande rend un résumé final. Une suite qui se fige sans résumé n'est pas un aléa — c'est l'interblocage Whoosh décrit par la spec (`flock()` jamais relâché sur `data/whoosh_index/MAIN_WRITELOCK`, boucle `whoosh/writing.py:1012`), qui **masque** les échecs au lieu de les afficher. Dans ce cas, ne rien conclure du décompte : reprendre fichier par fichier, dans des processus frais, en purgeant le verrou entre deux, puis s'arrêter et remonter le fait.

Consigner les quatre valeurs dans `$SCRATCH/preuves-i3.txt` (le répertoire existe depuis T1) : nombre de tests, couverture, nombre de modules `mypy`, résultat fonctionnel.
...
### T9 — l'image part de `python:3.14-alpine`, et uwsgi est bâti contre cet interpréteur
...
**Interfaces:**
- Consomme : le venv 3.14 de T8 (pour `make check` avant commit) ; rien du côté du code.
...
**Régime de preuve : exécution réelle.** Aucun test unitaire ne bâtit une image ni ne rejoue un `CMD`. La preuve est en trois constats dans le conteneur (Step 7) et dans le journal de démarrage (Step 8) ; la recette de l'incrément est T10.
...
**Interfaces:**
- Consomme : les images bâties en T9, `$SCRATCH/preuves-i3.txt`.
...
Chapitre 1 de `docs/recette.md`, depuis l'instance montée en T9 Step 8, sans écart : premier utilisateur `test`/`test`, cabinet, thérapeute, puis le patient `Picard Jean-Luc`, ses deux consultations, la facture `10000` à `55 €` et le document « Radiographie lombaire ».
...
Ajouter à `$SCRATCH/preuves-i3.txt` : les trois verdicts, le commit recetté (`git rev-parse HEAD`) et le tag d'images. **Ne rien écrire dans `docs/recette.md`** (le cahier ne se coche jamais) ni dans `KANBAN.md` (c'est T11).

**Critère de fin :** `R-INST-01`, `R-INST-02`, `R-INST-03` en OK, et `$SCRATCH/preuves-i3.txt` portant les huit constats de l'incrément (quatre de T9 Step 7, un de T9 Step 8, trois verdicts).
...
**Interfaces:**
- Consomme : `$SCRATCH/preuves-i3.txt` (T8 à T10), `preuves-i1.txt` (T1 à T4), `preuves-i2.txt` (T6, T7).
...
Les **49 fiches** — les 48 existantes plus `R-INST-06` — sont jouées, dans l'ordre que le chapitre 0 laisse libre, chacune depuis son état requis. `R-INST-06` demande de reconstituer un E2 sur PostgreSQL 13 (protocole de T4 Step 1) : la jouer en dernier, sur son propre montage, pour ne pas contraindre les 48 autres.
...
2. **Ce que le lot a appris et qui n'était pas su au cadrage** — au minimum :
   - **la suite a tourné pour la première fois sur l'interpréteur de production**, et ce que cela a révélé (rien, ou les défauts préexistants rendus visibles par T8, avec leur traitement) ;
   ...
   - **le comportement de Django 4.2.30 sur PostgreSQL 18**, état transitoire de l'incrément 1, constaté et non supposé ;
...
Section « Dette technologique — analyse automatisée du 2026-09-02, triée le 2026-09-04 » : **supprimer la puce « Élevé — socle hors support »**. Ses trois objets sont traités — Django 4.2 hors support (T6), Python de l'image non maîtrisé (T8, T9), PostgreSQL 13 en fin de vie (T1, T4). Si l'un d'eux s'avérait n'être traité qu'en partie, **réécrire la puce au lieu de la supprimer**, et dire dans l'entrée de clôture ce qui reste.
...
|---|---|
| I1 livrable 1 — `postgres:18-alpine`, point de montage, `.env.example` | T1 |
| I1 livrable 2 — la procédure de montée en six étapes, dans le `README.rst` | T2 |
| I1 livrable 3 — la purge de l'état E0 ; `R-INST-05` étape 2 amendée ; `R-INST-06` | T3 |
| I1 preuve — montée **exécutée** depuis E2, et les douze fiches de l'incrément | T4 |
| I2 livrable 2 — la ligne morte `default_app_config` | T5 |
| I2 livrable 1 — Django 5.2.17, les quatre tiers, `drf-excel`, `django-statici18n`, les stubs | T6 |
| I2 preuve — recette, dont `R-RCH-01`/`R-RCH-02`, le point exposé du lot | T7 |
| I3 livrable 4 — déclarations alignées (CI, `ruff`, `mypy`, `README.rst`), venv de développement en 3.14 | T8 |
| I3 livrable 1 — les deux `FROM` en `python:3.14-alpine`, `python3-dev`/`py3-pip` retirés | T9 |
| I3 livrable 2 — uwsgi compilé par `pip`, `.build-deps` recomposé, commentaire `:70-78` **réécrit** | T9 |
| I3 livrable 3 — le `CMD` perd `--plugin http,python` et rien d'autre | T9 |
| I3 livrable 5 — la phrase `libreosteo-sock` du `README.rst` | T9 |
| I3 preuve — recette `R-INST-01`, `R-INST-02`, `R-INST-03` | T10 |
| Passage complet du cahier ; quatre sorties ; renvois en « À faire » ; cliquets constatés | T11 |

Les numéros `I1`, `I2`, `I3` sont ceux de la spec **révisée** le 2026-09-05 : I1 = PostgreSQL, I2 = Django, I3 = Python.

| Critère d'acceptation | Où il se prouve |
|---|---|
| 1 — `make check` vert sur l'interpréteur que la CI déclare à ce commit (3.13 jusqu'à T7, 3.14 à partir de T8), cliquets tenus | dernière étape de chaque tâche ; T11 Step 3 pour le relevé, sous 3.14 |
| 2 — 272 unitaires et 31 fonctionnels, **sans qu'aucun ait été modifié** | T5, T6, T8 ; T11 Step 3, par un `git diff` sur les deux répertoires de tests |
| 3 — `python -V` en 3.14.x et `SHOW server_version` en 18 sur l'instance du chapitre 0 | T11 Step 1 |
| 4 — procédure de montée **exécutée** depuis E2, sans y ajouter un geste | T4 Step 2, rejouée en T11 Step 2 par `R-INST-06` |
| 5 — `R-INST-06` existe, a été jouée, aucune fiche renumérotée | T3 Step 5 (insertion), T4 Step 2 (jouée), T11 Step 2 |
| 6 — passage complet de `docs/recette.md`, fiche neuve incluse | T11 Step 2 |
| 7 — trois incréments dans l'ordre PostgreSQL → Django → Python, chacun déployable et recettable | T1 Step 5 et T4 ; T7 Step 1 ; T9 Step 8 et T10. Une fusion des incréments 1 et 2 serait une **révision écrite et motivée**, cf. T1 Step 5 |
