...
DEFAULT_JSON_URL = (
    "https://www.data.gouv.fr/en/datasets/r/34d4364c-22eb-4ac0-b179-7a1845ac033a"
)
...
class Command(BaseCommand):
    ...
    def handle(self, json_url, json_file, **options):
        print("Fetching zipcodes for FRANCE from {}".format(json_file or json_url))
        if options["download_only"]:
            open("zipcode_dataset.json", "w").write(
                urlopen(json_url).read().decode("utf-8")
            )
            print("Done")
            return
        if json_file:
        ...
        print(
            "Inserting zipcodes into database from {} {}".format(
